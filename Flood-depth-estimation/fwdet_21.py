...
class FwDET(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        param = QgsProcessingParameterString(self.grow_metric, 'r.grow.distance metric', defaultValue='euclidean',
                                             optional=False)
        ...
        self.addParameter(
            QgsProcessingParameterRasterDestination(self.OUTPUT_WSH_SMOOTH,
                                                    self.tr('Interpolated Water Depths Raster (with Low-Pass filter)'),
                                                    optional=True)
        )
    ...
    def processAlgorithm(self, params, context, feedback):
        ...
        inun_vlay_fp = \
        self._algo('native:savefeatures', {'INPUT': params[self.INUN_VLAY], 'OUTPUT': 'TEMPORARY_OUTPUT'})['OUTPUT']
    ...
    def run_algo(self, dem_rlay_raw, inun_vlay, numIterations, slopeTH, grow_distance,
        ...
        if inun_vlay.crs().isGeographic():
            feedback.pushWarning(
                f'{inun_vlay.name()}s CRS ({inun_vlay.crs()}) is Geographic. this may lead to unexpected results. consider re-projecting')
    ...
    def CalculateBoundary(self, dem_rlay, inun_vlay, numIterations, slopeTH,
        ...
        if numIterations > 0:
            ...
            for i in range(numIterations):
                ...
                neigh_fp = self._r_neighbors(boundary_fp_i, neighborhood_size=neighborhood_size, method='maximum')
        ...
        boundary1 = self._gdal_calc({'FORMULA': 'A*(B >-30)',
                                     'INPUT_A': boundary_fp_i, 'BAND_A': 1, 'INPUT_B': dem_min_fp, 'BAND_B': 1,
                                     'NO_DATA': 0.0, 'OUTPUT': self._get_out(self.OUTPUT_SHORE), 'RTYPE': 5})
        ...
        boundary3 = self._algo('native:roundrastervalues',
                               {'BAND': 1, 'BASE_N': 10, 'DECIMAL_PLACES': 4,
                                'INPUT': boundary2, 'OUTPUT': 'TEMPORARY_OUTPUT', 'ROUNDING_DIRECTION': 1}
                               )['OUTPUT']
    ...
    def _gdal_calc_mask(self, rlay, OUTPUT='TEMPORARY_OUTPUT'):
        return self._gdal_calc(
            {'FORMULA': 'A/A', 'INPUT_A': rlay, 'BAND_A': 1, 'NO_DATA': -9999, 'OUTPUT': OUTPUT, 'RTYPE': 5})
    ...
    def _r_neighbors(self, input_rlay,
        ...
        return self._algo('grass7:r.neighbors',
                          {'-a': False, '-c': circular,  # Use circular neighborhood
                           ...
                           'gauss': None,
                           'input': input_rlay,
                           'method': {'average': 0, 'minimum': 3, 'maximum': 4}[method],
